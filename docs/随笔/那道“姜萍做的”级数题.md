...
24年6月，江苏省涟水中等专业学校17岁服装专业学生姜萍以**93分、全球第12名**的成绩晋级阿里巴巴全球数学竞赛决赛，从而引发热议。
...
> 这些递推式不是每个都能用到，这只是思考过程。有用的是第一个递推式，因为它出现了$\csc x$
>
> 
> $$
...
$$
=\frac{1}{\pi^2}\left[\psi'\left(1-\frac{z}{\pi}\right)+\psi'\left(\frac{z}{\pi}\right)\right] =\frac{1}{\pi^2}\left[\pi^2\csc^2\left(\pi \cdot \frac{z}{\pi}\right)\right] = \csc^2 z
$$


这样就证明了级数恒等式：$\displaystyle \sum_{k=-\infty}^{+\infty}{\frac{1}{(k\pi+z)^{2}}} = \csc^2 z$

最后令$\displaystyle z=\frac{\omega}{2}$，就证好了。
...
同样令$\displaystyle x=\frac{\omega}{2}$，原式得证。
...
**解：**函数是周期为$\displaystyle \frac{2\pi}{a}$的偶函数，因此可将$f$延拓为数轴上周期为$2\pi$的函数。计算系数如下：
...
$$
b_n = 0\quad (n=1,2,\cdots) \quad \quad \leftarrow \text{因为是偶函数}
$$
...
$$
\cot x = \frac{1}{x} + \frac{1}{\pi}\sum_{n=1}^{\infty}{\frac{2\left(\frac{x}{\pi}\right)}{\left(\frac{x}{\pi}\right)^2-n^2}} = \frac{1}{x}+\sum_{n=1}^{\infty}{\frac{2x}{x^2-(n\pi)^2}}
$$
